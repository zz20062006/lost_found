...
<div class="container-fluid"><div class="row">
    <div class="col-md-2 admin-sidebar">
        <nav class="nav flex-column">
            ...
            <a class="nav-link active" href="lost-manage.jsp"><i class="fas fa-exclamation-circle mr-2"></i>失物审核</a>
    ...
    <div class="col-md-10 admin-content">
        <h4 class="mb-3"><i class="fas fa-exclamation-circle"></i> 失物审核管理</h4>
        <div class="card mb-3"><div class="card-body">
            <form class="form-inline" id="filterForm">
                ...
                <select class="form-control mr-2 mb-1" id="status">
                    ...
                    <option value="1">寻找中</option><option value="2">已找回</option>
                </select>
                <button type="submit" class="btn btn-primary btn-sm mb-1">筛选</button>
            </form>
        </div></div>
...
<script>
    ...
    function loadData(page) {
        ...
        var params = {action:'lostList', page:currentPage,
        ...
        $.getJSON(contextPath + '/AdminServlet', params, function(res){
            ...
            $.each(res.data, function(i, item){
                html += '<tr>';
                html += '<td>'+item.itemId+'</td><td>'+item.title+'</td><td>'+item.category+'</td>';
    ...
    function renderPage(res) {
        ...
        if (res.totalPage > 1) {
            ...
            for(var p=1; p<=res.totalPage; p++) {
                ph += '<li class="page-item '+(p==res.currentPage?'active':'')+'"><a class="page-link" href="javascript:void(0)" onclick="loadData('+p+')">'+p+'</a></li>';
            }
    ...
    function approve(id) {
        $.post(contextPath + '/AdminServlet', {action:'lostApprove', itemId:id}, function(res){
    ...
    function reject(id) {
        ...
        $.post(contextPath + '/AdminServlet', {action:'lostReject', itemId:id}, function(res){
            showToast(res.msg, res.code===1?'success':'error');
            if(res.code===1) loadData();
        }, 'json');
    }
    function del(id) {
        ...
        $.post(contextPath + '/AdminServlet', {action:'lostDelete', itemId:id}, function(res){
            showToast(res.msg, res.code===1?'success':'error');
            if(res.code===1) loadData();
        }, 'json');
    }
